...
namespace {
  ...
  class FreeContext : public Command {
  public:
    FreeContext( Worker & worker ) : Command( false, worker ) {
    }
	...
	l->add( m_toks->ptr( 0 ) );
	l->add( "OK" );
	l->add( std::string( "I have eaten your context." ) );
  ...
  Command::Register<FreeContext> r( "FREECONTEXT", AUTH );
}
