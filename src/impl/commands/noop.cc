...
namespace {
  ...
  public:
    Noop( Worker & worker ) : Command( false, worker ) {
    }
	...
	throw std::runtime_error( "Too few arguments." );
      ...
      if( m_toks->length() > 2 ) {
	throw std::runtime_error( "If you ever wondered why NOOP could return BAD, now you know." );
      }
    }
    ...
    void main() {
      ...
      l->add( m_toks->ptr( 0 ) );
      l->add( "OK" );
      l->add( std::string( "I really hope you didn't expect anything interesting to happen." ) );
      m_worker.send( l, true );
    }
    
  };
  
  Command::Register<Noop> r( "NOOP", INIT );
}
